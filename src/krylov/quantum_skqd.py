...
class QuantumCircuitSKQD:
    ...
    def _build_cudaq_kernels(self) -> None:
        ...
        Note: Both cudaq.State and list[complex] state-caching approaches were
        attempted but neither produces correct evolution on the nvidia/fp64 target
        (CUDA-Q v0.12). cudaq.get_state(kernel, cudaq.State, ...) returns the
        input unchanged; cudaq.qvector(list[complex]) + exp_pauli also fails to
        evolve. Using proven from-scratch cudaq.sample() for correctness.
        """
    ...
    def _sample_cudaq(self, krylov_power: int) -> Dict[str, int]:
        ...
        Note: State-caching was attempted via both cudaq.State and list[complex]
        amplitude passing, but neither produces correct evolution on nvidia/fp64
        (CUDA-Q v0.12). Using proven from-scratch approach for correctness.
        """
        self._init_cudaq()
        ...
        if not hasattr(self, "_cudaq_info_printed"):
            ...
            print(f"  CUDA-Q from-scratch mode ({n_terms} Pauli terms, "
                  f"dim={dim:,}, O(k²) total Trotter steps)")
