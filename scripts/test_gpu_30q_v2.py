#!/usr/bin/env python3
"""Test HI+NQS+SQD on 24Q-30Q with no basis size limit."""
...
for mol_name, nq, n_samp in molecules:
    ...
    if not info.get("is_cas", False):
        ccsd_r = CCSDTSolver().solve(H, info)
        if ccsd_r.energy:
            print(f"  CCSD(T)= {ccsd_r.energy:.10f} Ha")

    sci_r = CIPSISolver(max_basis_size=10000).solve(H, info)
    if sci_r.energy:
        print(f"  SCI    = {sci_r.energy:.10f} Ha (basis={sci_r.diag_dim})")

    # HI+NQS+SQD (no basis limit)
    np.random.seed(42)
    ...
    ref_e = fci_r.energy or (sci_r.energy if sci_r.energy else None)
    if r.energy and ref_e:
