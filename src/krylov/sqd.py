...
class SQDSolver:
    ...
    def _recover_configurations(
        ...
        needing the same number of flips are processed together via batched
        torch.multinomial (which accepts a (n_group, n_orb) weight matrix and
        returns (n_group, n_flip) sampled indices in one call).
        """
        ...
        def _fix_sector_vectorized(
            ...
            sector_slice: (n_configs, n_orb) — the slice of recovered to update
            occ:          (n_orb,)           — orbital occupancies for this sector
            target_n:     int                — desired particle count
            counts:       (n_configs,) long  — current particle count per config
            Returns the updated sector_slice (modified in place, also returned).
            """
            diffs = counts - target_n          # positive → too many, negative → too few
            needs_fix = diffs != 0
            ...
            for diff_val in unique_diffs:
                ...
                group_idx = group_mask.nonzero(as_tuple=True)[0]  # absolute row indices
                group = sector_slice[group_idx]  # (n_group, n_orb)
                n_flip = abs(diff)

                if diff > 0:
                    # Too many particles: flip occupied → unoccupied.
                    # Weight non-zero only for occupied orbitals (==1).
                    # distance = |1 - occ[j]|  (high when occ is low → should be empty)
                    occ_row = occ.unsqueeze(0).expand(group.size(0), -1)  # (n_group, n_orb)
                    distances = torch.abs(1.0 - occ_row)
                    # Zero out weight for empty orbitals — we can only flip occupied ones
                    weights = self._modified_relu(distances, delta, h) * group.float()
                    weights = weights / (weights.sum(dim=1, keepdim=True) + 1e-12)

                    # Clamp n_flip to min occupied count across group
                    ...
                    # flip_cols: (n_group, n_flip_actual) — column indices to set to 0
                    rows = torch.arange(group.size(0), device=device).unsqueeze(1).expand_as(flip_cols)
                    group[rows, flip_cols] = 0

                else:
                    # Too few particles: flip unoccupied → occupied.
                    # distance = |0 - occ[j]|  (high when occ is high → should be occupied)
                    occ_row = occ.unsqueeze(0).expand(group.size(0), -1)  # (n_group, n_orb)
                    distances = torch.abs(occ_row)
                    # Zero out weight for occupied orbitals — we can only flip empty ones
                    weights = self._modified_relu(distances, delta, h) * (1 - group.float())
                    weights = weights / (weights.sum(dim=1, keepdim=True) + 1e-12)

                    # Clamp n_flip to min empty count across group
                ...
                # Write corrected group back into sector_slice
        ...
        recovered[:, :n_orb] = _fix_sector_vectorized(alpha_slice, occ_alpha, self.n_alpha, alpha_counts)

        # Fix beta sector
        beta_slice = recovered[:, n_orb:]
        recovered[:, n_orb:] = _fix_sector_vectorized(beta_slice, occ_beta, self.n_beta, beta_counts)
    ...
    def _compute_s2_matrix(self, configs: torch.Tensor) -> torch.Tensor:
        ...
        # Chunked GPU computation: process rows in blocks to bound memory to
        # O(chunk_size * n * n_orb) instead of O(n^2 * n_orb).
        chunk_size = min(n, max(500, 4_000_000 // (n * n_orb + 1)))  # ~16 MB per chunk

        for i_start in range(0, n, chunk_size):
            i_end = min(i_start + chunk_size, n)

            alpha_chunk = alpha[i_start:i_end]  # (chunk_n, n_orb)
            beta_chunk = beta[i_start:i_end]    # (chunk_n, n_orb)

            # (chunk_n, n, n_orb)
            alpha_diff = alpha_chunk.unsqueeze(1) - alpha.unsqueeze(0)
            beta_diff = beta_chunk.unsqueeze(1) - beta.unsqueeze(0)

            a_plus = (alpha_diff == 1).sum(dim=2)    # (chunk_n, n)
            ...
            a_plus_pos = (alpha_diff == 1).float().argmax(dim=2)   # (chunk_n, n)
            ...
            valid = count_match & site_match  # (chunk_n, n)

            # Restrict to strict upper triangle (col > row in the full matrix)
            ...
            valid_upper = valid & upper_mask  # (chunk_n, n)

            # Write -1 to both triangles for each valid pair
